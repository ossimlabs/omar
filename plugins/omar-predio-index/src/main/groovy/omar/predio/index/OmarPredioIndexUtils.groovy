...
@Slf4j
class OmarPredioIndexUtils
{
   private static ConfigObject predioIndexConfig
    ...
    */
   static synchronized ConfigObject getPredioIndexConfig() {
      if (predioIndexConfig == null) {
         log.trace 'Building predio index config since there is no cached config'
         reloadPredioIndexConfig()
      }
   ...
   /** Force a reload of the predio index configuration. */
   static void reloadPredioIndexConfig() {
      mergeConfig OmarPredioIndexReflectionUtils.getPredioIndexConfig(), 'DefaultOmarPredioIndexConfig'

      mergeConfigToGlobalConfig()

      log.trace 'reloaded predio index config'
   }
   /** Reset the config for testing or after a dev mode Config.groovy change. */
   static synchronized void resetPredioIndexConfig() {
      predioIndexConfig = null
      log.trace 'reset predio index config'
   }
...
//      mergeConfig this.securityConfig.spring,  SpringSecurityUtils.securityConfig
   ...
   private static void mergeConfig(ConfigObject currentConfig, String className) {
      ...
      predioIndexConfig = OmarPredioIndexReflectionUtils.predioIndexConfig = mergeConfig(currentConfig, secondary.predio.index as ConfigObject)
   }
